...
public class JFrameExplorar extends JFramePrincipal {
	...
	private void inicializarPanelCentral() {
		...
		opciones.addActionListener((e) -> filtrarLibros());
	...
	public void filtrarLibros() {
		...
		for (Libro libro : libros) {
			...
			if (!reservado) {
				...
				if (tit.isEmpty()
						|| libro.getTitulo().toLowerCase().contains(tit)
						|| libro.getAutor().toLowerCase().contains(tit)) {
		...
		if (listaLibros.isEmpty()) {
			...
			for (Libro l : listaLibros) {
				...
				// =======================
				// NUEVO: Panel info con Chip de género + valoración
				// =======================
				...
				autor.setBorder(BorderFactory.createEmptyBorder(0, 10, 6, 10));
				panelInfo.add(autor);

				JPanel panelBadges = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
				panelBadges.setBorder(BorderFactory.createEmptyBorder(0, 10, 10, 10));
				panelBadges.setBackground(noSeleccionado);
				panelBadges.setOpaque(true);

				// Chip de género
				GenreChip chipGenero = new GenreChip(l.getGenero());
				panelBadges.add(chipGenero);

				// Valoración
				JLabel valoracion = new JLabel("★ " + String.format("%.2f", l.getValoracion()), JLabel.LEFT);
				valoracion.setFont(new Font("SansSerif", Font.BOLD, 13));
				panelBadges.add(valoracion);
